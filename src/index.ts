...
app.post('/co-pilot', async (req: Request, res: Response) => {
    ...
    const suggestionResponse = JSON.parse((await suggestion(process.env.API_KEY))!)
    ...
    const response = await getOperationCalldata({
        ...
        tokenOut: suggestionResponse[0].liquidityPoolAddress,
        routingStrategy: 'delegate',
    });
    res.send({ tx: response.tx, suggestion: suggestionResponse[0].suggestion });
